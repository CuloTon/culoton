...
FOMO_LINES = [
    "TON is quietly eating 2026. BRAINROT publishes the receipts. $BRT is along for the ride.",
    "Three previous chains, one narrative. $BRT came home — and home is on TON.",
    "We don't shill TON. We publish three roundups a day about it. Coincidence? No. $BRT",
    "Telegram is the chat. TON is the chain. $BRT is the meme that ties them.",
    "Sub-second blocks on TON. Sub-280-character takes from BRAINROT. $BRT is the punchline.",
    "Most projects shill. BRAINROT publishes. Then we mention $BRT — lightly.",
    "If TON wins, $BRT wins. Track record: 10,000X on Polygon, then SUI, now TON.",
    "Small cap. Big chain. Attentive crew. $BRT on TON — and we're just clearing our throat.",
    "Don't bet on TON. Be on it. $BRT is one of many ways.",
    "Build the media voice first, the bag follows. BRAINROT → $BRT → TON. In that order.",
    "While altcoins fight for relevance, TON ships and BRAINROT writes it down. $BRT funds the desk.",
    "The fastest chain in crypto runs the most-used messenger on Earth. $BRT is the meme leg.",
]
...
def post_mcap(client, delivery: str) -> int:
    ...
    if not data or data.get("price") is None:
        print("Could not fetch $BRT data from GeckoTerminal — skipping (no-op).", file=sys.stderr)
        return 0
