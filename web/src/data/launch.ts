...
const EN: LaunchContent = {
  meta_title: 'BRAINROT Launcher — Deploy your TON jetton in 60 seconds',
  ...
  why_points: [
    ['Niche is wide open', 'Standalone web jetton-launchers with logo upload, editable metadata and a clean revoke flow barely exist. The few that do are bare and ugly. The audience already trusts BRAINROT for TON-ecosystem coverage — this is the natural next step.'],
  ...
  pricing_disclaimer: 'You always pay gas directly to the TON network through your own wallet. The service fee is bundled in the same transaction and goes to the BRAINROT treasury — used to fund the news desk and the weekly community prize pool.',
  ...
  faq: [
    ...
    { q: 'What about the logo if your site goes down?', a: 'The logo and metadata live on IPFS, content-addressed. Even if brainrot-ton.fun disappears tomorrow, your jetton still shows correctly in every wallet — TON wallets can fetch IPFS directly.' },
  ...
  disclaimer: 'BRAINROT Launcher does not offer financial advice, custody your funds, or vet the tokens deployed through it. Anyone can deploy a token on TON — verify before you trust or buy. The renounce-ownership feature is irreversible by design.',
};

const RU: LaunchContent = {
  meta_title: 'BRAINROT Launcher — деплой TON-jetton за 60 секунд',
  ...
  why_points: [
    ['Ниша открыта', 'Standalone веб-лаунчеров jetton’ов с загрузкой логотипа, редактируемыми метаданными и нормальным revoke практически нет. Те что есть — голые и страшные. Аудитория BRAINROT уже доверяет нам по освещению экосистемы TON — это логичный следующий шаг.'],
  ...
  pricing_disclaimer: 'Газ всегда платится напрямую сети TON через ваш кошелёк. Сервисная комиссия идёт в той же транзакции в казну BRAINROT — на финансирование редакции и еженедельного призового фонда сообщества.',
  ...
  faq: [
    ...
    { q: 'Что с логотипом, если ваш сайт упадёт?', a: 'Логотип и метаданные на IPFS, адресуются по содержимому. Даже если brainrot-ton.fun исчезнет завтра — jetton всё равно правильно отображается в каждом кошельке. TON-кошельки умеют ходить в IPFS напрямую.' },
  ...
  disclaimer: 'BRAINROT Launcher не даёт финансовых советов, не хранит ваши средства и не проверяет токены, задеплоенные через него. Задеплоить токен на TON может кто угодно — проверяйте перед тем, как доверять или покупать. Функция отказа от владения необратима по дизайну.',
};

const PL: LaunchContent = {
  meta_title: 'BRAINROT Launcher — wdróż swój jetton TON w 60 sekund',
  ...
  why_points: [
    ['Nisza jest otwarta', 'Standalone webowych launcherów jettonów z wgraniem logo, edytowalnymi metadanymi i czystym revoke prawie nie ma. Te które są — surowe i brzydkie. Publiczność BRAINROT już ufa nam w newsach o ekosystemie TON — to naturalny następny krok.'],
  ...
  pricing_disclaimer: 'Gas zawsze płacisz bezpośrednio do sieci TON, przez swój portfel. Prowizja serwisowa idzie w tej samej transakcji do skarbca BRAINROT — wykorzystywana na finansowanie redakcji i tygodniowej puli nagród dla społeczności.',
  ...
  faq: [
    ...
    { q: 'A co z logo jeśli wasza strona padnie?', a: 'Logo i metadane lezą na IPFS, adresowane po treści. Nawet jeśli brainrot-ton.fun zniknie jutro, twój jetton dalej pokazuje się poprawnie w każdym portfelu. Portfele TON potrafią chodzić do IPFS bezpośrednio.' },
  ...
  disclaimer: 'BRAINROT Launcher nie udziela porad finansowych, nie przechowuje twoich środków i nie weryfikuje tokenów wdrożonych przez niego. Każdy może wdrożyć token na TON — sprawdź zanim zaufasz lub kupisz. Funkcja renounce ownership jest nieodwracalna by design.',
};

const DE: LaunchContent = {
  meta_title: 'BRAINROT Launcher — TON-Jetton in 60 Sekunden deployen',
  ...
  why_points: [
    ['Nische offen', 'Eigenständige Web-Jetton-Launcher mit Logo-Upload, editierbaren Metadaten und sauberem Revoke-Flow gibt es kaum. Die wenigen existierenden sind nackt und hässlich. Das BRAINROT-Publikum vertraut uns schon bei TON-Berichterstattung — das ist der natürliche nächste Schritt.'],
  ...
  pricing_disclaimer: 'Gas zahlst du immer direkt ans TON-Netzwerk über dein eigenes Wallet. Die Service-Gebühr ist in derselben Transaktion gebündelt und geht an die BRAINROT-Treasury — finanziert die News-Redaktion und den wöchentlichen Community-Preispool.',
  ...
  faq: [
    ...
    { q: 'Was passiert mit dem Logo, wenn eure Seite ausfällt?', a: 'Logo und Metadaten leben auf IPFS, inhalts-adressiert. Selbst wenn brainrot-ton.fun morgen verschwindet, zeigt dein Jetton in jedem Wallet weiterhin korrekt — TON-Wallets können IPFS direkt abrufen.' },
  ...
  disclaimer: 'BRAINROT Launcher bietet keine Finanzberatung, verwahrt eure Mittel nicht und prüft die deployten Token nicht. Jeder kann einen Token auf TON deployen — prüft, bevor ihr vertraut oder kauft. Die Renounce-Funktion ist by design unwiderruflich.',
};

const ES: LaunchContent = {
  meta_title: 'BRAINROT Launcher — despliega tu jetton de TON en 60 segundos',
  ...
  why_points: [
    ['Nicho abierto', 'Launchers de jetton web independientes con subida de logo, metadatos editables y un flujo de revoke limpio apenas existen. Los pocos que hay son pelados y feos. La audiencia de BRAINROT ya confía en nosotros para cobertura del ecosistema TON — este es el siguiente paso natural.'],
  ...
  pricing_disclaimer: 'Siempre pagas el gas directamente a la red TON a través de tu propia billetera. La comisión de servicio va en la misma transacción y entra en la treasury de BRAINROT — se usa para financiar la redacción y el premio comunitario semanal.',
  ...
  faq: [
    ...
    { q: '¿Qué pasa con el logo si vuestro sitio se cae?', a: 'El logo y los metadatos viven en IPFS, direccionados por contenido. Incluso si brainrot-ton.fun desaparece mañana, tu jetton se sigue mostrando correctamente en cada billetera — las billeteras TON pueden ir a IPFS directamente.' },
  ...
  disclaimer: 'BRAINROT Launcher no ofrece asesoramiento financiero, no custodia tus fondos ni examina los tokens desplegados a través de él. Cualquiera puede desplegar un token en TON — verifica antes de confiar o comprar. La función de renounce ownership es irreversible por diseño.',
};

const UK: LaunchContent = {
  meta_title: 'BRAINROT Launcher — деплой TON-джетона за 60 секунд',
  ...
  why_points: [
    ['Ніша відкрита', 'Самостійні веб-лаунчери джетонів із завантаженням лого, редагованими метаданими й нормальним revoke майже не існують. Ті що є — голі й страшні. Аудиторія BRAINROT уже довіряє нам по освітленню екосистеми TON — це логічний наступний крок.'],
  ...
  pricing_disclaimer: 'Газ завжди платиться напряму мережі TON через ваш гаманець. Сервісна комісія йде в тій самій транзакції в скарбницю BRAINROT — на фінансування редакції та щотижневого призового фонду спільноти.',
  ...
  faq: [
    ...
    { q: 'А що з лого, якщо ваш сайт впаде?', a: 'Лого й метадані на IPFS, адресуються за вмістом. Навіть якщо brainrot-ton.fun зникне завтра — джетон все одно правильно відображається в кожному гаманці. TON-гаманці вміють ходити в IPFS напряму.' },
  ...
  disclaimer: 'BRAINROT Launcher не дає фінансових порад, не зберігає ваші кошти й не перевіряє токени, задеплоєні через нього. Задеплоїти токен на TON може будь-хто — перевіряйте перш ніж довіряти чи купувати. Функція відмови від володіння незворотна за дизайном.',
};
